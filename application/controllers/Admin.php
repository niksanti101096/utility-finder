...
class Admin extends REST_Controller {
    ...
    public function create_new_lead_post(){
        if($this->session->userdata('uf_session')){
            $sess = $this->session->userdata('uf_session');
            $data = [
                // 'new_lead_id' => $this->get('new_lead_id') ? $this->get('new_lead_id') : '',
                'lead_id' => substr(str_shuffle(str_repeat("0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ", 3)), 0, 5),
